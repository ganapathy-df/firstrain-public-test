...
(function($) {
    ...
    Rectangle.prototype.style = function(hasSubtree) {
        return {
            top: this.y + 'px',
            left: this.x + 'px',
            width: (hasSubtree ? this.width : (this.width - this.margin)) + "px",
            height: (hasSubtree ? this.height : (this.height - this.margin)) + "px"
        };
    }

    Rectangle.prototype.isWide = function() {
        return ((this.width * 2) / 3) > this.height;
    }

    function TreeMap($div, options,internal) {
        var options = options || {};
        this.$div = $div;

        if (!internal) {$div.css('position', 'relative');} // added by MSB for placing treemaps in treemaps
        ...
        this.setNodeColors = function($box, node) { // added node here (MSB)
            //if (this.backgroundColor) $box.css('background-color', this.backgroundColor(node));
            //if (this.color) $box.css('color', this.color(node));
        }
		this.addMouseEvents=function($box, node) { // Added by msb
			//$box.mouseleave(function () {$box.removeClass("treeNodeHover"); return false;});
			//$box.mouseenter(function () {$box.addClass("treeNodeHover"); return false;});
		}
    }

    TreeMap.SIDE_MARGIN = 2;
    TreeMap.TOP_MARGIN = 2;

    TreeMap.prototype.paint = function(nodeList,options) {
        ...
        for (var i = 0; i < nodeList.length; i++) {
            ...
            $box.css($.extend(nodeBounds.style(node.subtree), {
                'position' : 'absolute',
                'overflow' : 'hidden'
            }));

            this.setNodeColors($box, node); // added node here(MSB)
			this.addMouseEvents($box, node); //added by MSB
			...
			// added to place subtrees: MSB
			if (node.subtree) {
				$box=$('#'+node.id);
				//$box.css('border', '1px white solid');
				//$box.css('background-color','white'); 
				$box.unbind();
				//$box.removeClass('gradient',''); 
				$box.treemap(node.subtree,options, true); 
			} else {
				//$box.addClass('gradient');
				$box.addClass(this.nodeClass(node, $box));

				var $content = $("<div><span>" + node.label + "</span></div>");
				$content.addClass('treemap-label');
				$content.css({
					'position': 'relative',
					'text-align': 'center',
					'vertical-align' : 'middle',
					'font-size': options.labelFontSize ? options.labelFontSize : '16px'
				});
				$box.append($content);
				if(!options.labelFontSize) {
					this.fitLabelFontSize($content, node, options);
				}
				var marginTop = (parseInt($box.height()) / 2) - (parseInt($content.height()) / 2);
				if(marginTop > 0) {
    ...
    TreeMap.prototype.fitLabelFontSize = function($content, node, options) {
    	var font;
     	if(node.part >= 20) {
     		font = options.fonts[4];
         } else if(node.part >= 15) {
	        font = options.fonts[3];
         } else if(node.part >= 10) {
	        font = options.fonts[2];
         } else if(node.part >= 5) {
         	font = options.fonts[1];
         } else {
         	font = options.fonts[0];
         }
     	$content.css('font-size', font.s + 'px').css('font-weight', font.w);
     	/*
        var nodeBounds = node.bounds;
        while ($content[0].firstChild.offsetHeight + TreeMap.TOP_MARGIN > nodeBounds.height 
        		|| $content[0].firstChild.offsetWidth + TreeMap.SIDE_MARGIN > nodeBounds.width 
        		|| $content[0].firstChild.scrollWidth + TreeMap.SIDE_MARGIN + 2 > nodeBounds.width) {
            var fontSize = parseFloat($content.css('font-size')) - 3;
            if (fontSize < 10) {
            	$content[0].innerHTML = '<span>...</span>';
//                $content.remove();
                break;
            }
    ...
    $.fn.treemap = function(json, options, internal) {
        var self = this;
        if(options.sync) {
        	return new TreeMap(self, options, internal).paint(json,options);
        } else {
	        return this.fadeOut('fast', function() {
	            self.empty().fadeIn('fast', function() {
	                new TreeMap(self, options, internal).paint(json,options);
	            });
	        });
        }
    };
