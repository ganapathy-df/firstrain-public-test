...
<c:choose>
	<c:when test="${header['Accept'] eq 'application/json'}">
		...
		<c:choose>
			<c:when test="${invalidIP}">
				{"message": "<%=resource.getString("errorcode.403")%>", "status": "ERROR", "errorCode": 403, "version": "${version}"}
			</c:when>
			<c:when test="${invalidVersion}">
				{"message": "<%=resource.getString("errorcode.410")%>", "status": "ERROR", "errorCode": 410, "version": "${version}"}
			</c:when>
			<c:when test="${unauthorized}">
				{"message": "<%=resource.getString("errorcode.105")%>", "status": "ERROR", "errorCode": 105, "version": "${version}"}
			</c:when>
			<c:when test="${errorCode eq 500}">
				{"message": "<%=resource.getString("errorcode.500")%>", "status": "ERROR", "errorCode": 500, "version": "${version}"}
			</c:when>
			<c:when test="${errorCode eq 404}">
				{"message": "<%=resource.getString("errorcode.404")%>", "status": "ERROR", "errorCode": 404, "version": "${version}"}
			</c:when>
			<c:when test="${errorCode eq 400}">
				{"message": "<%=resource.getString("errorcode.400")%>", "status": "ERROR", "errorCode": 400, "version": "${version}"}
			</c:when>
			<c:when test="${errorCode eq 405}">
				{"message": "<%=resource.getString("errorcode.405")%>", "status": "ERROR", "errorCode": 405, "version": "${version}"}
			</c:when>
			<c:otherwise>
				{"message": "<%=resource.getString("errorcode.500")%>", "status": "ERROR", "errorCode": 500, "version": "${version}"}
			</c:otherwise>
	...
	<c:otherwise>
		...
		<body>
			<div class="err-msg">
				<p class="pd">Oops Something is broken. Please email to feedback@firstrain.com so we can investigate.</p></div>
