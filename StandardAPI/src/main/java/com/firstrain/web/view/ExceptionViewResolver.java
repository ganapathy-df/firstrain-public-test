...
public class ExceptionViewResolver extends SimpleMappingExceptionResolver {
		...
		if (handler != null) {
			try {
				...
				if (e instanceof PermissionException) {
					...
					populate400ErrorResponse(e, handler, model, request, response);
				} else if (e instanceof HttpRequestMethodNotSupportedException) {
					populate405ErrorResponse(e, handler, model, request, response);
				} else {
	...
	private void populate400ErrorResponse(final Exception e, final Object handler, final Map<String, Object> model, final HttpServletRequest request, final HttpServletResponse response) {
		populateErrorResponse(request, handler, e, response, 400, model);
	}

	private void populate405ErrorResponse(final Exception e, final Object handler, final Map<String, Object> model, final HttpServletRequest request, final HttpServletResponse response) {
		populateErrorResponse(request, handler, e, response, 405, model);
	}

	private void populateErrorResponse(final HttpServletRequest request, final Object handler, final Exception e, final HttpServletResponse response, final int code, final Map<String, Object> model) {
		logRequestDetails(request, handler, e);
		response.setStatus(code);
		request.setAttribute("errorCode", code);
		setErrorMessageForJSONRes(model, request, code);
	}
